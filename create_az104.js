...
function modulo3(imgs) { return [
    ...
    bullet('Cache locale — il server mantiene una cache dei nomi di dominio usati di recente con i relativi indirizzi IP. Se la risposta e\' in cache, la restituisce subito senza contattare altri server. Se non la trova, passa la richiesta a un altro server DNS e cosi\' via fino alla corrispondenza o al timeout.'),
    bullet('Autorita\' su una zona — il server gestisce il database delle coppie nome/IP per tutti gli host e sottodomini su cui ha autorita\'. Questa funzione e\' tipicamente associata a Web, posta elettronica e altri servizi Internet del dominio.'),
    ...
    infoBox('SOA e origine di autorita\':','Quando si configura DNS di Azure come SOA (Start of Authority) per il proprio dominio, Azure diventa il punto di riferimento autorevole per quel dominio su Internet. Gli altri server DNS che ricevono richieste per quel dominio lo interrogheranno come fonte di verita\'.'),
    ...
    bullet('Zone DNS private — risoluzione dei nomi per VM in reti virtuali senza esporre i record su Internet. Supporta split-horizon DNS (stesso dominio risolto diversamente da dentro o fuori la VNet).'),
    bullet('Record alias — i record DNS possono puntare direttamente a risorse Azure (IP pubblico, Traffic Manager, CDN, Front Door) e si aggiornano automaticamente al variare dell\'IP della risorsa.'),
    ...
    body('Una zona DNS pubblica ospita i record DNS di un dominio rendendoli visibili su Internet. I passi per configurarla sono:'),
    stepTitle('Passo 1 — Creare la zona DNS in Azure'),
    body('Nel portale Azure si crea una nuova risorsa "Zona DNS". I parametri richiesti sono: sottoscrizione, gruppo di risorse, nome del dominio (es. contoso.com) e area del gruppo di risorse (impostata automaticamente).'),
    ...
    body('Accedere al pannello di gestione del registrar del dominio e sostituire i server dei nomi esistenti con i quattro forniti da Azure. Questa operazione si chiama delega del dominio e puo\' richiedere da 10 minuti ad alcune ore per propagarsi.'),
    stepTitle('Passo 4 — Verificare la delega'),
    body('Usare nslookup per verificare che la delegazione sia avvenuta correttamente interrogando il record SOA:'),
    ...
    bullet('Record CNAME — crea un alias verso un altro nome. Es. www -> contoso.com con TTL 600s. Usato per funzioni di Azure e altri servizi PaaS.'),
    ...
    body('Individuare le VNet in cui risiedono le VM che devono poter risolvere i nomi privati. I nomi delle VNet sono necessari per creare i collegamenti.'),
    stepTitle('Passo 3 — Collegare la VNet alla zona privata'),
    body('Nella zona DNS privata, selezionare "Collegamenti di rete virtuale" -> "Aggiungi" e scegliere la VNet da collegare. Ripetere per ogni VNet che deve partecipare alla risoluzione dei nomi privati.'),
    bullet('I nomi host delle VM nella VNet vengono mantenuti automaticamente nella zona privata.'),
    bullet('Il supporto split-horizon consente di avere lo stesso nome di dominio in zona pubblica e privata, risolto diversamente in base alla sorgente della richiesta.'),
    ...
    body('I record CNAME non possono essere usati a livello di dominio apex. Questo crea un problema quando si vuole puntare contoso.com (senza www) verso un servizio come Traffic Manager o un CDN, che richiedono un nome invece di un IP fisso.'),
    stepTitle('Soluzione — record alias'),
    body('I record alias di Azure permettono a un record sull\'apex di zona (tipo A, AAAA o CNAME) di puntare direttamente a una risorsa Azure invece che a un indirizzo IP statico. Il collegamento e\' dinamico: se l\'IP della risorsa cambia, il record DNS si aggiorna automaticamente.'),
    ...
    bullet('Endpoint di Azure CDN (Content Delivery Network) — per distribuzione dei contenuti geograficamente distribuita.'),
