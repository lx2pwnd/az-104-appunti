...
const imgs = {
  ...
  route5SysRoutes:  loadImg('Module 3 - Configurare e gestire reti virtuali/route5-system-routes.png'),
  route5VnetUdrs:   loadImg('Module 3 - Configurare e gestire reti virtuali/route5-vnet-peering-udrs.png'),
  route5Gateway:    loadImg('Module 3 - Configurare e gestire reti virtuali/route5-vnet-gateway.png'),
  route5Bgp:        loadImg('Module 3 - Configurare e gestire reti virtuali/route5-bgp.png'),
  route5Nva:        loadImg('Module 3 - Configurare e gestire reti virtuali/route5-nva.png'),
};
...
function sommario() {
  return [
    ...
    tocHeading('3.5 — Gestire e controllare il flusso del traffico nella distribuzione di Azure tramite route'),
    tocEntry('3.5.1 — Introduzione'),
    tocEntry('3.5.2 — Identificare le funzionalita\' di routing di una rete virtuale di Azure'),
    tocEntry('3.5.3 — Che cos\'e\' un\'appliance virtuale di rete?'),
...
function modulo3(imgs) { return [
    ...
    // ─── 3.5 — Gestire e controllare il flusso del traffico ──────────────────
    h2('3.5 — Gestire e controllare il flusso del traffico nella distribuzione di Azure tramite route'),
    h3('3.5.1 — Introduzione'),
    body('Una rete virtuale consente di implementare un perimetro di sicurezza per le risorse nel cloud. E\' possibile controllare le informazioni che passano attraverso una rete virtuale e limitare l\'accesso per consentire solo il traffico proveniente da origini attendibili.'),
    body('Scenario tipico: un\'organizzazione di rivendita al dettaglio ha subito un evento imprevisto per la sicurezza con esposizione di dati dei clienti. Utenti malintenzionati hanno sfruttato vulnerabilita\' nell\'infrastruttura di rete. Come risposta, il team di sicurezza ha consigliato di aggiungere appliance virtuali di rete e il team cloud deve assicurarsi che il traffico venga instradato correttamente attraverso di esse.'),
    stepTitle('Obiettivi di apprendimento'),
    bullet('Identificare le funzionalita\' di routing di una rete virtuale di Azure.'),
    bullet('Configurare il routing in una rete virtuale.'),
    bullet('Distribuire un\'appliance virtuale di rete di base.'),
    bullet('Configurare il routing per inviare il traffico attraverso un\'appliance virtuale di rete.'),
    infoBox('Prerequisiti:','Conoscenza dei concetti fondamentali delle reti (subnet e indirizzi IP). Familiarita\' con le reti virtuali di Azure.'),

    h3('3.5.2 — Identificare le funzionalita\' di routing di una rete virtuale di Azure'),
    body('Per controllare il flusso del traffico all\'interno della rete virtuale e\' necessario conoscere lo scopo delle route personalizzate e come configurarle per indirizzare il traffico attraverso un\'appliance virtuale di rete.'),
    stepTitle('Routing di Azure — Route di sistema'),
    body('Il traffico di rete in Azure viene instradato automaticamente tra subnet, reti virtuali e reti locali tramite route di sistema. Queste route sono assegnate per impostazione predefinita a ogni subnet e consentono alle VM di comunicare tra loro e potenzialmente con l\'ambiente locale e Internet.'),
    body('Non e\' possibile creare o eliminare route di sistema, ma e\' possibile eseguirne l\'override aggiungendo route personalizzate.'),
    stepTitle('Route di sistema predefinite per ogni subnet'),
    makeTable(
    ...
    body('Il tipo di hop successivo determina il percorso del traffico: **Rete virtuale** crea route per ogni intervallo di indirizzi della VNet; **Internet** instrada tutto il traffico non corrispondente verso Internet; **Nessuno** elimina il traffico verso gli intervalli privati non instradabili a livello globale.'),
    ...figImg(imgs.route5SysRoutes,'png',550,390,'Figura 44 — Flusso del traffico tra subnet e Internet con le route di sistema predefinite: le subnet comunicano liberamente e il traffico non interno raggiunge Internet.'),
    stepTitle('Route di sistema aggiuntive'),
    body('Azure crea route di sistema aggiuntive quando si abilitano le seguenti funzionalita\':'),
    stepTitle('Peering di rete virtuale e concatenamento dei servizi'),
    body('Il peering consente l\'interconnessione delle reti virtuali nella stessa area o in aree diverse, creando route aggiuntive nella tabella di route. Il concatenamento dei servizi consente di sostituire tali route con route definite dall\'utente tra reti con peering.'),
    ...figImg(imgs.route5VnetUdrs,'png',550,347,'Figura 45 — Peering di rete virtuale con route definite dall\'utente: il traffico viene instradato attraverso un\'appliance virtuale di rete o un Gateway VPN di Azure.'),
    stepTitle('Gateway di rete virtuale'),
    body('Un gateway di rete virtuale invia traffico crittografato tra Azure e l\'ambiente locale tramite Internet, o tra reti Azure. Contiene le tabelle di routing e i servizi gateway.'),
    ...figImg(imgs.route5Gateway,'png',550,291,'Figura 46 — Struttura di un gateway di rete virtuale: connette la rete Azure all\'ambiente locale o ad altre reti tramite VPN crittografata.'),
    body('Gli endpoint di rete virtuale estendono lo spazio indirizzi privato fornendo connessione diretta alle risorse Azure, limitando il flusso e impedendo l\'accesso da VM pubbliche.'),
    stepTitle('Route personalizzate'),
    body('Le route di sistema semplificano la gestione dell\'ambiente, ma in molti scenari occorre un controllo piu\' preciso — ad esempio per instradare il traffico attraverso una NVA o un firewall. Le route personalizzate offrono questo controllo.'),
    stepTitle('Route definite dall\'utente (UDR)'),
    body('Una UDR sovrascrive le route di sistema predefinite per instradare il traffico attraverso firewall o appliance virtuali di rete. Quando si creano UDR e\' possibile specificare i seguenti tipi di hop successivo:'),
    makeTable(
    ...
    stepTitle('Tag di servizio per le route definite dall\'utente'),
    body('E\' possibile specificare un tag di servizio (anziche\' un intervallo IP esplicito) come prefisso per una UDR. Un tag di servizio rappresenta un gruppo di prefissi IP di un servizio Azure e viene aggiornato automaticamente da Microsoft, riducendo la complessita\' e il numero di route da gestire.'),
    stepTitle('Protocollo BGP (Border Gateway Protocol)'),
    body('Un gateway di rete locale puo\' scambiare route con un gateway di rete virtuale in Azure usando BGP. BGP e\' il protocollo di routing standard per scambiare informazioni tra sistemi autonomi. Si usa tipicamente con Azure ExpressRoute o connessioni VPN da sito a sito.'),
    ...figImg(imgs.route5Bgp,'png',550,198,'Figura 47 — Topologia BGP: percorsi che consentono il passaggio dei dati tra il Gateway VPN di Azure e le reti locali tramite il protocollo Border Gateway.'),
    body('BGP offre stabilita\' della rete: i router modificano rapidamente le connessioni se un percorso non e\' disponibile.'),
    stepTitle('Selezione delle route e priorita\''),
    body('Se in una tabella di route sono disponibili piu\' route, Azure usa quella con la corrispondenza del prefisso piu\' lungo (prefisso specifico > generico). Non e\' possibile configurare piu\' UDR con lo stesso prefisso.'),
    body('Se piu\' route condividono lo stesso prefisso, Azure seleziona la route in base al tipo con il seguente ordine di priorita\':'),
    bullet('1. Route definite dall\'utente (priorita\' massima)'),
    bullet('2. Route BGP'),
    bullet('3. Route di sistema (priorita\' minima)'),

    h3('3.5.3 — Che cos\'e\' un\'appliance virtuale di rete?'),
    body('Un\'appliance virtuale di rete (NVA) e\' una VM virtuale multistrato che puo\' svolgere le seguenti funzioni:'),
    bullet('Firewall'),
    bullet('Utilita\' di ottimizzazione WAN'),
    bullet('Controller per la distribuzione delle applicazioni'),
    bullet('Router'),
    bullet('Servizi di bilanciamento del carico'),
    bullet('Sistemi di rilevamento/prevenzione intrusioni (IDS/IPS)'),
    bullet('Proxy'),
    body('E\' possibile distribuire NVA da Azure Marketplace (Cisco, Check Point, Barracuda, Sophos, WatchGuard, SonicWall) per filtrare il traffico in ingresso verso una rete virtuale e bloccare richieste dannose o provenienti da risorse impreviste.'),
    stepTitle('Architettura con appliance virtuale di rete'),
    body('Le NVA sono VM che controllano il flusso del traffico gestendo il routing. Vengono in genere usate per gestire il flusso tra un ambiente di rete perimetrale e altre reti o subnet.'),
    ...figImg(imgs.route5Nva,'png',550,367,'Figura 48 — Architettura di rete con NVA: l\'appliance virtuale controlla il traffico tra la rete perimetrale, le subnet applicative e Internet.'),
    stepTitle('Microsegmentazione'),
    body('Con l\'approccio basato su microsegmentazione e\' possibile creare subnet dedicate per il firewall e distribuire applicazioni Web e altri servizi in altre subnet. Tutto il traffico viene instradato attraverso il firewall e controllato dalla NVA. Si abilita l\'inoltro per le interfacce di rete dell\'appliance per passare il traffico accettato alla subnet appropriata.'),
    body('La microsegmentazione consente al firewall di ispezionare tutti i pacchetti al livello OSI 4 e, per appliance compatibili con le applicazioni, al livello 7. Quando distribuita in Azure, la NVA funge da router, inoltrando le richieste tra le subnet.'),
    stepTitle('Route definite dall\'utente per NVA'),
    body('Per la maggior parte degli ambienti le route di sistema predefinite sono sufficienti. In alcuni casi e\' necessario creare tabelle di routing con route personalizzate:'),
    bullet('Accesso a Internet tramite la rete locale mediante tunneling forzato.'),
    bullet('Uso di appliance virtuali per controllare il flusso del traffico.'),
    body('E\' possibile creare piu\' tabelle di route in Azure: ogni tabella puo\' essere associata a una o piu\' subnet, ma una subnet puo\' essere associata a una sola tabella di route.'),
    stepTitle('Alta disponibilita\' delle NVA'),
    body('Se il traffico e\' instradato attraverso una NVA, questa diventa un componente critico dell\'infrastruttura: eventuali errori influiscono direttamente sulla capacita\' di comunicazione dei servizi. E\' importante includere un\'architettura a disponibilita\' elevata nella distribuzione delle NVA.'),
    infoBox('Nota:','Alcune NVA richiedono piu\' interfacce di rete: una dedicata alla gestione e altre per l\'elaborazione del traffico. Dopo la distribuzione, la NVA viene configurata per instradare il traffico attraverso l\'interfaccia appropriata.'),
  ]; }
